# Revision brief — Book 19 essay 02 (apply the panel review)

Essay 02 of `/home/diablo/book19` (`chapters/02-border-files.html`, `checks/claims/02.tsv`,
`resources/sources/02/`) has been panel-reviewed. The consolidated review is
`drafts/reviews/02-codex.md`; the two underlying reports are `drafts/reviews/02-flash.md`
and `02-pro.md` (the consolidator already adjudicated them; where it rejected a reviewer's
suggestion, leave that alone).

Work only inside `/home/diablo/book19` and only in the essay-02 files plus
...
Then run `python3 checks/structure.py 02` and `python3 checks/claims.py 02` until both
pass, walk AGENT.md's pre-ship test, and write `drafts/reviews/02-applied.md`: one numbered
