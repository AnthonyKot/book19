# Revision brief — Book 19 essay 05 (apply the panel review)

Essay 05 of `/home/diablo/book19` (`chapters/05-owner-independent-acquisition.html`, `checks/claims/05.tsv`,
`resources/sources/05/`) has been panel-reviewed. The consolidated review is
`drafts/reviews/05-codex.md`; the two underlying reports are `drafts/reviews/05-flash.md`
and `05-pro.md` (the consolidator already adjudicated them; where it rejected a reviewer's
suggestion, leave that alone).

Work only inside `/home/diablo/book19` and only in the essay-05 files plus
...
Then run `python3 checks/structure.py 05` and `python3 checks/claims.py 05` until both
pass, walk AGENT.md's pre-ship test, and write `drafts/reviews/05-applied.md`: one numbered
