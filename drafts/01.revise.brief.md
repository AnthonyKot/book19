# Revision brief — Book 19 essay 01 (apply the panel review)

Essay 01 of `/home/diablo/book19` (`chapters/01-cell-file.html`, `checks/claims/01.tsv`,
`resources/sources/01/`) has been panel-reviewed. The consolidated review is
`drafts/reviews/01-codex.md`; the two underlying reports are `drafts/reviews/01-flash.md`
and `01-pro.md` (the consolidator already adjudicated them; where it rejected a reviewer's
suggestion, leave that alone).

Work only inside `/home/diablo/book19` and only in the essay-01 files plus
...
Then run `python3 checks/structure.py 01` and `python3 checks/claims.py 01` until both
pass, walk AGENT.md's pre-ship test, and write `drafts/reviews/01-applied.md`: one numbered
